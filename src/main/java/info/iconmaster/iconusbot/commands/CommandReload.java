...
public class CommandReload extends Command {
	...
	public CommandReload() {
		super("reload", "Reloads the global save file.", "USAGE: !reload\n"
				+ "This command reloads the global save file, `userdata.json`. Useful for making manual changes without restarting the bot.", true);
	}
	
	@Override
	public void execute(UserData user, IChannel channel, String[] args) {
		IconusBot.INSTANCE.readUserData();
		IconusBot.INSTANCE.sendMessage(channel, "`userdata.json` successfully reloaded.");
	}
}
